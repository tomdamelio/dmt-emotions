# -*- coding: utf-8 -*-
"""
Unified RSP_Amplitude (Respiratory Amplitude) Analysis: LME modeling and visualization (first 9 minutes).

This script processes respiration-derived respiratory amplitude (RSP_Amplitude) from NeuroKit RSP_Amplitude column:
  1) Build long-format per-minute RSP_Amplitude dataset (mean amplitude per minute, 0–8)
...
Outputs are written to: results/resp/amplitude/

Run:
  python scripts/run_resp_amplitude_analysis.py
"""
...
def load_resp_csv(csv_path: str) -> Optional[pd.DataFrame]:
    ...
    if 'time' not in df.columns:
        sr = NEUROKIT_PARAMS.get('sampling_amplitude_default', 250)
        df['time'] = np.arange(len(df)) / float(sr)
    
    return df


def compute_amplitude_mean_per_minute(t: np.ndarray, amplitude: np.ndarray, minute: int) -> Optional[float]:
    """Compute mean RSP_Amplitude for a specific minute window."""
    start_time = minute * 60.0
    end_time = (minute + 1) * 60.0
    mask = (t >= start_time) & (t < end_time)
    if not np.any(mask):
        return None
    amplitude_win = amplitude[mask]
    amplitude_win_valid = amplitude_win[(amplitude_win > 0) & (amplitude_win < 3000)]  # Physiological range (arbitrary units)
    if len(amplitude_win_valid) < 2:
        return None
    return float(np.nanmean(amplitude_win_valid))


def prepare_long_data_resp_amplitude() -> pd.DataFrame:
    """Build long-format per-minute RSP_Amplitude table (first 9 minutes)."""
    rows: List[Dict] = []
    for subject in SUJETOS_VALIDADOS_RESP:
        ...
        # Extract time and RSP_Amplitude
        t_dmt_high = dmt_high['time'].to_numpy()
        amplitude_dmt_high = pd.to_numeric(dmt_high['RSP_Amplitude'], errors='coerce').to_numpy()
        t_dmt_low = dmt_low['time'].to_numpy()
        amplitude_dmt_low = pd.to_numeric(dmt_low['RSP_Amplitude'], errors='coerce').to_numpy()
        t_rs_high = rs_high['time'].to_numpy()
        amplitude_rs_high = pd.to_numeric(rs_high['RSP_Amplitude'], errors='coerce').to_numpy()
        t_rs_low = rs_low['time'].to_numpy()
        amplitude_rs_low = pd.to_numeric(rs_low['RSP_Amplitude'], errors='coerce').to_numpy()

        # Optional baseline correction
        if BASELINE_CORRECTION:
            # Subtract mean of first minute
            baseline_dmt_high = compute_amplitude_mean_per_minute(t_dmt_high, amplitude_dmt_high, 0)
            baseline_dmt_low = compute_amplitude_mean_per_minute(t_dmt_low, amplitude_dmt_low, 0)
            baseline_rs_high = compute_amplitude_mean_per_minute(t_rs_high, amplitude_rs_high, 0)
            baseline_rs_low = compute_amplitude_mean_per_minute(t_rs_low, amplitude_rs_low, 0)
            if baseline_dmt_high is not None:
                amplitude_dmt_high = amplitude_dmt_high - baseline_dmt_high
            if baseline_dmt_low is not None:
                amplitude_dmt_low = amplitude_dmt_low - baseline_dmt_low
            if baseline_rs_high is not None:
                amplitude_rs_high = amplitude_rs_high - baseline_rs_high
            if baseline_rs_low is not None:
                amplitude_rs_low = amplitude_rs_low - baseline_rs_low

        for minute in range(N_MINUTES):
            amplitude_dmt_h = compute_amplitude_mean_per_minute(t_dmt_high, amplitude_dmt_high, minute)
            amplitude_dmt_l = compute_amplitude_mean_per_minute(t_dmt_low, amplitude_dmt_low, minute)
            amplitude_rs_h = compute_amplitude_mean_per_minute(t_rs_high, amplitude_rs_high, minute)
            amplitude_rs_l = compute_amplitude_mean_per_minute(t_rs_low, amplitude_rs_low, minute)
            
            if None not in (amplitude_dmt_h, amplitude_dmt_l, amplitude_rs_h, amplitude_rs_l):
                minute_label = minute + 1  # store minutes as 1..9 instead of 0..8
                rows.extend([
                    {'subject': subject, 'minute': minute_label, 'Task': 'DMT', 'Dose': 'High', 'RSP_Amplitude': amplitude_dmt_h},
                    {'subject': subject, 'minute': minute_label, 'Task': 'DMT', 'Dose': 'Low', 'RSP_Amplitude': amplitude_dmt_l},
                    {'subject': subject, 'minute': minute_label, 'Task': 'RS', 'Dose': 'High', 'RSP_Amplitude': amplitude_rs_h},
                    {'subject': subject, 'minute': minute_label, 'Task': 'RS', 'Dose': 'Low', 'RSP_Amplitude': amplitude_rs_l},
                ])

    if not rows:
        raise ValueError('No valid RSP_Amplitude data found for any subject!')
...
def fit_lme_model(df: pd.DataFrame) -> Tuple[Optional[object], Dict]:
    ...
    try:
        formula = 'RSP_Amplitude ~ Task * Dose + minute_c + Task:minute_c + Dose:minute_c'
...
def generate_report(fitted_model, diagnostics: Dict, hypothesis_results: Dict, df: pd.DataFrame, output_dir: str) -> str:
    lines: List[str] = [
        '=' * 80,
        'LME ANALYSIS REPORT: RSP_Amplitude (Respiratory Amplitude) by Minute (first 9 minutes)',
        '=' * 80,
        '',
        ...
        '  Dependent variable: Mean RSP_Amplitude per minute (a.u.)',
        f'  Baseline correction: {BASELINE_CORRECTION}',
        '',
        'MODEL SPECIFICATION:',
        '  Fixed effects: RSP_Amplitude ~ Task*Dose + minute_c + Task:minute_c + Dose:minute_c',
    ...
    cell = df.groupby(['Task', 'Dose'], observed=False)['RSP_Amplitude'].agg(['count', 'mean', 'std']).round(4)
    lines.extend(['Cell means (RSP_Amplitude by Task × Dose):', str(cell), ''])
    trend = df.groupby('minute', observed=False)['RSP_Amplitude'].agg(['count', 'mean', 'std']).round(4)
    lines.extend(['Time trend (RSP_Amplitude by minute):', str(trend), ''])
    lines.extend(['', '=' * 80])
...
def compute_empirical_means_and_ci(df: pd.DataFrame, confidence: float = 0.95) -> pd.DataFrame:
    grouped = df.groupby(['minute', 'Task', 'Dose'], observed=False)['RSP_Amplitude']
...
def create_marginal_means_plot(stats_df: pd.DataFrame, output_path: str) -> None:
    ...
    ax.set_ylabel('RSP_Amplitude (a.u.)')
...
def create_interaction_plot(stats_df: pd.DataFrame, output_path: str) -> None:
    ...
    ax1.set_ylabel('RSP_Amplitude (a.u.)')
...
def create_effect_sizes_table(coef_df: pd.DataFrame, output_path: str) -> None:
    ...
    table['interpretation'] = table.apply(lambda r: (
        ...
        f"{'increase' if r['beta'] > 0 else 'decrease'} in RSP_Amplitude"), axis=1)
...
def create_model_summary_txt(diagnostics: Dict, coef_df: pd.DataFrame, output_path: str) -> None:
    lines: List[str] = [
        ...
        'RSP_Amplitude ~ Task*Dose + minute_c + Task:minute_c + Dose:minute_c',
        '',
        'Random Effects: ~ 1 | subject',
        '',
        'Model Fit Statistics:',
        ...
        for subject in SUJETOS_VALIDADOS_RESP:
            try:
                if kind == 'DMT':
                    ...
                    yh = pd.to_numeric(d_high['RSP_Amplitude'], errors='coerce').to_numpy()
                    tl = d_low['time'].to_numpy()
                    yl = pd.to_numeric(d_low['RSP_Amplitude'], errors='coerce').to_numpy()
                else:  # RS
                    high_session, low_session = determine_sessions(subject)
                    ...
                    yh = pd.to_numeric(r_high['RSP_Amplitude'], errors='coerce').to_numpy()
                    tl = r_low['time'].to_numpy()
                    yl = pd.to_numeric(r_low['RSP_Amplitude'], errors='coerce').to_numpy()
...
def create_dmt_only_20min_plot(out_dir: str) -> Optional[str]:
    ...
    Saves results/resp/amplitude/all_subs_dmt_resp_hr.png
    """
...
def create_stacked_subjects_plot(out_dir: str) -> Optional[str]:
    ...
    for subject in SUJETOS_VALIDADOS_RESP:
        try:
            ...
            yr1 = pd.to_numeric(r1['RSP_Amplitude'], errors='coerce').to_numpy()
            tr2 = r2['time'].to_numpy()
            yr2 = pd.to_numeric(r2['RSP_Amplitude'], errors='coerce').to_numpy()
    ...
    for i, row in enumerate(rows):
        ...
        ax_rs.set_ylabel('RSP_Amplitude (a.u.)', fontsize=STACKED_AXES_LABEL_SIZE)
        ...
        ax_dmt.set_ylabel('RSP_Amplitude (a.u.)', fontsize=STACKED_AXES_LABEL_SIZE)
...
def generate_captions_file(output_dir: str) -> None:
    captions = [
        'Figure: LME Coefficients (RSP_Amplitude)\n\n'
        ...
        'Group-level mean ± 95% CI of RSP_Amplitude (a.u.) across the first 9 minutes for each condition (RS Low/High, DMT Low/High). '
        ...
        '',
        'Figure: Group-level RSP_Amplitude Timecourse (9 min)\n\n'
        ...
        '',
        'Figure: DMT-only RSP_Amplitude Timecourse (~19 min)\n\n'
        ...
        '',
        'Figure: Stacked Per-Subject RSP_Amplitude Timecourse (9 min)\n\n'
    ...
    with open(os.path.join(output_dir, 'captions_resp_amplitude.txt'), 'w', encoding='utf-8') as f:
        f.write('\n\n'.join(captions))


def main() -> bool:
    out_dir = os.path.join('results', 'resp', 'amplitude')
    plots_dir = os.path.join(out_dir, 'plots')
    os.makedirs(plots_dir, exist_ok=True)
    try:
        # Data
        print("Preparing long-format RSP_Amplitude data...")
        df = prepare_long_data_resp_amplitude()
        df.to_csv(os.path.join(out_dir, 'resp_amplitude_minute_long_data.csv'), index=False)
        ...
        print(f"\nRespiration amplitude analysis complete! Results in: {out_dir}")
    except Exception as e:
        print(f'Respiration amplitude analysis failed: {e}')
        import traceback
        traceback.print_exc()
        return False
    return True
