# -*- coding: utf-8 -*-
"""
Unified RSP_Symmetry (Respiratory Symmetry) Analysis: LME modeling and visualization (first 9 minutes).

This script processes respiration-derived respiratory symmetry from NeuroKit RSP_Symmetry columns:
  1) Build long-format per-minute RSP_Symmetry dataset (mean symmetry per minute, 0–8)
     Symmetry = average of PeakTrough and RiseDecay symmetry metrics
...
Outputs are written to: results/resp/symmetry/

Run:
  python scripts/run_resp_symmetry_analysis.py
"""
...
def load_resp_csv(csv_path: str) -> Optional[pd.DataFrame]:
    ...
    # Validate required columns - need both symmetry metrics
    if 'RSP_Symmetry_PeakTrough' not in df.columns or 'RSP_Symmetry_RiseDecay' not in df.columns:
        return None
    
    # Compute average symmetry from both metrics
    pt = pd.to_numeric(df['RSP_Symmetry_PeakTrough'], errors='coerce')
    rd = pd.to_numeric(df['RSP_Symmetry_RiseDecay'], errors='coerce')
    df['RSP_Symmetry'] = (pt + rd) / 2.0
    
    # Reconstruct time if missing
    if 'time' not in df.columns:
        sr = NEUROKIT_PARAMS.get('sampling_symmetry_default', 250)
        df['time'] = np.arange(len(df)) / float(sr)
    
    return df


def compute_symmetry_mean_per_minute(t: np.ndarray, symmetry: np.ndarray, minute: int) -> Optional[float]:
    """Compute mean RSP_Symmetry for a specific minute window."""
    start_time = minute * 60.0
    end_time = (minute + 1) * 60.0
    mask = (t >= start_time) & (t < end_time)
    if not np.any(mask):
        return None
    symmetry_win = symmetry[mask]
    symmetry_win_valid = symmetry_win[(symmetry_win >= 0) & (symmetry_win <= 1)]  # Physiological range (proportion)
    if len(symmetry_win_valid) < 2:
        return None
    return float(np.nanmean(symmetry_win_valid))


def prepare_long_data_resp_symmetry() -> pd.DataFrame:
    """Build long-format per-minute RSP_Symmetry table (first 9 minutes)."""
    rows: List[Dict] = []
    for subject in SUJETOS_VALIDADOS_RESP:
        ...
        # Extract time and RSP_Symmetry
        t_dmt_high = dmt_high['time'].to_numpy()
        symmetry_dmt_high = pd.to_numeric(dmt_high['RSP_Symmetry'], errors='coerce').to_numpy()
        t_dmt_low = dmt_low['time'].to_numpy()
        symmetry_dmt_low = pd.to_numeric(dmt_low['RSP_Symmetry'], errors='coerce').to_numpy()
        t_rs_high = rs_high['time'].to_numpy()
        symmetry_rs_high = pd.to_numeric(rs_high['RSP_Symmetry'], errors='coerce').to_numpy()
        t_rs_low = rs_low['time'].to_numpy()
        symmetry_rs_low = pd.to_numeric(rs_low['RSP_Symmetry'], errors='coerce').to_numpy()

        # Optional baseline correction
        if BASELINE_CORRECTION:
            # Subtract mean of first minute
            baseline_dmt_high = compute_symmetry_mean_per_minute(t_dmt_high, symmetry_dmt_high, 0)
            baseline_dmt_low = compute_symmetry_mean_per_minute(t_dmt_low, symmetry_dmt_low, 0)
            baseline_rs_high = compute_symmetry_mean_per_minute(t_rs_high, symmetry_rs_high, 0)
            baseline_rs_low = compute_symmetry_mean_per_minute(t_rs_low, symmetry_rs_low, 0)
            if baseline_dmt_high is not None:
                symmetry_dmt_high = symmetry_dmt_high - baseline_dmt_high
            if baseline_dmt_low is not None:
                symmetry_dmt_low = symmetry_dmt_low - baseline_dmt_low
            if baseline_rs_high is not None:
                symmetry_rs_high = symmetry_rs_high - baseline_rs_high
            if baseline_rs_low is not None:
                symmetry_rs_low = symmetry_rs_low - baseline_rs_low

        for minute in range(N_MINUTES):
            symmetry_dmt_h = compute_symmetry_mean_per_minute(t_dmt_high, symmetry_dmt_high, minute)
            symmetry_dmt_l = compute_symmetry_mean_per_minute(t_dmt_low, symmetry_dmt_low, minute)
            symmetry_rs_h = compute_symmetry_mean_per_minute(t_rs_high, symmetry_rs_high, minute)
            symmetry_rs_l = compute_symmetry_mean_per_minute(t_rs_low, symmetry_rs_low, minute)
            
            if None not in (symmetry_dmt_h, symmetry_dmt_l, symmetry_rs_h, symmetry_rs_l):
                minute_label = minute + 1  # store minutes as 1..9 instead of 0..8
                rows.extend([
                    {'subject': subject, 'minute': minute_label, 'Task': 'DMT', 'Dose': 'High', 'RSP_Symmetry': symmetry_dmt_h},
                    {'subject': subject, 'minute': minute_label, 'Task': 'DMT', 'Dose': 'Low', 'RSP_Symmetry': symmetry_dmt_l},
                    {'subject': subject, 'minute': minute_label, 'Task': 'RS', 'Dose': 'High', 'RSP_Symmetry': symmetry_rs_h},
                    {'subject': subject, 'minute': minute_label, 'Task': 'RS', 'Dose': 'Low', 'RSP_Symmetry': symmetry_rs_l},
                ])

    if not rows:
        raise ValueError('No valid RSP_Symmetry data found for any subject!')
...
def fit_lme_model(df: pd.DataFrame) -> Tuple[Optional[object], Dict]:
    ...
    try:
        formula = 'RSP_Symmetry ~ Task * Dose + minute_c + Task:minute_c + Dose:minute_c'
...
def generate_report(fitted_model, diagnostics: Dict, hypothesis_results: Dict, df: pd.DataFrame, output_dir: str) -> str:
    lines: List[str] = [
        '=' * 80,
        'LME ANALYSIS REPORT: RSP_Symmetry (Respiratory Symmetry) by Minute (first 9 minutes)',
        '=' * 80,
        '',
        ...
        '  Dependent variable: Mean RSP_Symmetry per minute (proportion)',
        f'  Baseline correction: {BASELINE_CORRECTION}',
        '',
        'MODEL SPECIFICATION:',
        '  Fixed effects: RSP_Symmetry ~ Task*Dose + minute_c + Task:minute_c + Dose:minute_c',
    ...
    cell = df.groupby(['Task', 'Dose'], observed=False)['RSP_Symmetry'].agg(['count', 'mean', 'std']).round(4)
    lines.extend(['Cell means (RSP_Symmetry by Task × Dose):', str(cell), ''])
    trend = df.groupby('minute', observed=False)['RSP_Symmetry'].agg(['count', 'mean', 'std']).round(4)
    lines.extend(['Time trend (RSP_Symmetry by minute):', str(trend), ''])
    lines.extend(['', '=' * 80])
...
def compute_empirical_means_and_ci(df: pd.DataFrame, confidence: float = 0.95) -> pd.DataFrame:
    grouped = df.groupby(['minute', 'Task', 'Dose'], observed=False)['RSP_Symmetry']
...
def create_marginal_means_plot(stats_df: pd.DataFrame, output_path: str) -> None:
    ...
    ax.set_ylabel('RSP_Symmetry (proportion)')
...
def create_interaction_plot(stats_df: pd.DataFrame, output_path: str) -> None:
    ...
    ax1.set_ylabel('RSP_Symmetry (proportion)')
...
def create_effect_sizes_table(coef_df: pd.DataFrame, output_path: str) -> None:
    ...
    table['interpretation'] = table.apply(lambda r: (
        ...
        f"{'increase' if r['beta'] > 0 else 'decrease'} in RSP_Symmetry"), axis=1)
...
def create_model_summary_txt(diagnostics: Dict, coef_df: pd.DataFrame, output_path: str) -> None:
    lines: List[str] = [
        ...
        'RSP_Symmetry ~ Task*Dose + minute_c + Task:minute_c + Dose:minute_c',
        '',
        'Random Effects: ~ 1 | subject',
        '',
        'Model Fit Statistics:',
        ...
        for subject in SUJETOS_VALIDADOS_RESP:
            try:
                if kind == 'DMT':
                    ...
                    yh = pd.to_numeric(d_high['RSP_Symmetry'], errors='coerce').to_numpy()
                    tl = d_low['time'].to_numpy()
                    yl = pd.to_numeric(d_low['RSP_Symmetry'], errors='coerce').to_numpy()
                else:  # RS
                    high_session, low_session = determine_sessions(subject)
                    ...
                    yh = pd.to_numeric(r_high['RSP_Symmetry'], errors='coerce').to_numpy()
                    tl = r_low['time'].to_numpy()
                    yl = pd.to_numeric(r_low['RSP_Symmetry'], errors='coerce').to_numpy()
...
def create_dmt_only_20min_plot(out_dir: str) -> Optional[str]:
    ...
    Saves results/resp/symmetry/all_subs_dmt_resp_hr.png
    """
...
def create_stacked_subjects_plot(out_dir: str) -> Optional[str]:
    ...
    for subject in SUJETOS_VALIDADOS_RESP:
        try:
            ...
            yr1 = pd.to_numeric(r1['RSP_Symmetry'], errors='coerce').to_numpy()
            tr2 = r2['time'].to_numpy()
            yr2 = pd.to_numeric(r2['RSP_Symmetry'], errors='coerce').to_numpy()
    ...
    for i, row in enumerate(rows):
        ...
        ax_rs.set_ylabel('RSP_Symmetry (proportion)', fontsize=STACKED_AXES_LABEL_SIZE)
        ...
        ax_dmt.set_ylabel('RSP_Symmetry (proportion)', fontsize=STACKED_AXES_LABEL_SIZE)
...
def generate_captions_file(output_dir: str) -> None:
    captions = [
        'Figure: LME Coefficients (RSP_Symmetry)\n\n'
        ...
        'Group-level mean ± 95% CI of RSP_Symmetry (proportion) across the first 9 minutes for each condition (RS Low/High, DMT Low/High). '
        ...
        '',
        'Figure: Group-level RSP_Symmetry Timecourse (9 min)\n\n'
        ...
        '',
        'Figure: DMT-only RSP_Symmetry Timecourse (~19 min)\n\n'
        ...
        '',
        'Figure: Stacked Per-Subject RSP_Symmetry Timecourse (9 min)\n\n'
    ...
    with open(os.path.join(output_dir, 'captions_resp_symmetry.txt'), 'w', encoding='utf-8') as f:
        f.write('\n\n'.join(captions))


def main() -> bool:
    out_dir = os.path.join('results', 'resp', 'symmetry')
    plots_dir = os.path.join(out_dir, 'plots')
    os.makedirs(plots_dir, exist_ok=True)
    try:
        # Data
        print("Preparing long-format RSP_Symmetry data...")
        df = prepare_long_data_resp_symmetry()
        df.to_csv(os.path.join(out_dir, 'resp_symmetry_minute_long_data.csv'), index=False)
        ...
        print(f"\nRespiration symmetry analysis complete! Results in: {out_dir}")
    except Exception as e:
        print(f'Respiration symmetry analysis failed: {e}')
        import traceback
        traceback.print_exc()
        return False
    return True
