...
class TETPhysioCCAAnalyzer:
    ...
    def _generate_all_derangements(self, n: int) -> list:
        """
        ...
        Returns:
            List of all derangements, each as a tuple of indices
        """
        from itertools import permutations
        
        # Filter permutations to keep only derangements (no fixed points)
        derangements = [
    ...
    def _apply_derangement(
        ...
        """
        Apply a specific derangement to shuffle Y data across subjects.
        
        Args:
            X: Physiological matrix (n_obs × 3)
            Y: TET affective matrix (n_obs × 6)
            subject_ids: Subject identifiers for each observation
            derangement: Tuple specifying the permutation mapping
        ...
        # Create mapping: subject at position i gets Y data from subject at position derangement[i]
        subject_mapping = {
            unique_subjects[i]: unique_subjects[derangement[i]] 
    ...
    def _subject_level_shuffle(
        ...
        """
        Perform subject-level shuffling for permutation testing (legacy method).
        
        This method preserves within-subject temporal structure while
        randomly pairing physiological and TET data across subjects.
        
        Note: For exact permutation testing, use _apply_derangement with
        all derangements from _generate_all_derangements instead.
        ...
        # Get all derangements and pick one randomly
        all_derangements = self._generate_all_derangements(n_subjects)
        chosen_derangement = all_derangements[np.random.randint(len(all_derangements))]
        
        return self._apply_derangement(X, Y, subject_ids, chosen_derangement)
    ...
    def permutation_test(
        ...
        """
        Perform exact subject-level permutation testing for CCA using all derangements.
        
        This method validates CCA canonical correlation significance by
        exhaustively testing all possible subject pairings (derangements)
        while preserving within-subject temporal structure.
        
        For n=7 subjects, there are exactly 1854 derangements (permutations
        where no subject is paired with itself).
        
        Process:
        1. For each state (RS, DMT):
           a. Extract observed canonical correlations
           b. Generate all possible derangements
           c. For each derangement:
              - Apply subject pairing
              - Fit CCA on permuted data
              - Extract permuted canonical correlations
           d. Compute exact empirical p-values
        
        Args:
            n_permutations: Ignored (kept for API compatibility). 
                All derangements are used automatically.
            random_state: Ignored (exact test is deterministic)
        
        Returns:
            Dict mapping state to DataFrame with columns:
            - canonical_variate: 1, 2, ...
            - observed_r: Observed canonical correlation
            - permutation_p_value: Exact empirical p-value
            - n_permutations: Number of derangements tested
        ...
        for state in self.cca_models.keys():
            self.logger.info(f"Exact permutation testing for {state} state...")
            ...
            # Generate ALL derangements (exact permutation test)
            all_derangements = self._generate_all_derangements(n_subjects)
            n_derangements = len(all_derangements)
            
            self.logger.info(
                f"  Found {n_derangements} derangements for {n_subjects} subjects"
            )
            ...
            permuted_corrs = np.zeros((n_derangements, n_components))
            
            # Test ALL derangements (exact permutation test)
            for perm_idx, derangement in enumerate(all_derangements):
                # Apply this specific derangement
                X_perm, Y_perm = self._apply_derangement(X, Y, subject_ids, derangement)
                ...
                # Compute canonical correlations
                permuted_corrs[perm_idx] = np.array([
                    np.corrcoef(U_perm[:, i], V_perm[:, i])[0, 1]
                    for i in range(n_components)
                ])
                
                # Log progress every 200 derangements
                if (perm_idx + 1) % 200 == 0:
                    self.logger.info(
                        f"  Completed {perm_idx + 1}/{n_derangements} derangements"
                    )
            
            # Compute exact empirical p-values
            ...
            self.logger.info(
                f"Exact permutation testing complete for {state}: "
                f"p-values = {p_values} (tested {n_derangements} derangements)"
            )
    ...
    def _generate_all_derangements(self, n: int) -> list:
        ...
        Returns:
            List of tuples representing all derangements
        
        Example:
            For n=3: returns [(1,2,0), (2,0,1)] - the only 2 derangements
        """
        from itertools import permutations
        
        original = tuple(range(n))
        derangements = [
            perm for perm in permutations(original)
            if all(perm[i] != i for i in range(n))
        ]
        return derangements
