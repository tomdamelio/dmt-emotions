# -*- coding: utf-8 -*-
"""
Plot per-subject EDR absolute value comparison for EDA (first 10 minutes): DMT high vs DMT low.
...
- Extracts the EDR series and time (or synthesizes time from sampling rate)
- Trims both recordings to the first 10 minutes
- Computes mean absolute value of EDR (phasic magnitude)
- Plots the EDR absolute value with mean levels and SEM shading
- Saves to test/eda/scr

EDR represents the phasic (event-related) component of EDA, capturing transient responses
to stimuli. For EDR, absolute magnitude is more meaningful than AUC since it captures
the strength of phasic responses rather than cumulative area.

Usage:
  python test/plot_edr_auc.py
"""
...
def _plot_boxplot_2x2(records, out_path):
    ...
    _beautify_axes(ax, title='EDR Mean – 2×2 design (paired lines)', xlabel='Dose', ylabel='Mean EDR (first 10 min)', time_formatter=False)
    fig.tight_layout()
...
def load_cvx_edr(csv_path: str) -> Optional[Tuple[np.ndarray, np.ndarray]]:
    """Load time and EDR arrays from CVX decomposition CSV.

    Returns (t, edr) or None if missing/invalid.
    """
    ...
    if 'EDR' not in df.columns:
        print(f"⚠️  EDR column not found in {os.path.basename(csv_path)}")
        return None
    ...
    edr = pd.to_numeric(df['EDR'], errors='coerce').fillna(0.0).to_numpy()
    return t, edr


def compute_mean_value(y: np.ndarray) -> float:
    """Compute mean value of EDR signal."""
...
def compute_envelope(y: np.ndarray, sr: float, window_sec: float = 1.0) -> np.ndarray:
    ...
    For EDR, use shorter window than SMNA since it's more phasic.
    """
...
def _plot_combined_summary(out_path: str) -> bool:
    """Create a combined summary plot with DMT (top) and RS (bottom) subplots.
    Each subplot shows High vs Low mean ± SEM over time with legend in upper right.
    """
        ...
        for subject in SUJETOS_VALIDADOS_EDA:
            try:
                if kind == 'DMT':
                    ...
                    d_high = load_cvx_edr(p_high)
                    d_low = load_cvx_edr(p_low)
                    ...
                    r1 = load_cvx_edr(p_r1)
                    r2 = load_cvx_edr(p_r2)
                    if None in (r1, r2):
                        continue
    ...
    _beautify_axes(ax1, title='DMT – EDR (mean ± SEM, first 10 min)', 
                  xlabel='Time (mm:ss)', ylabel='EDR (a.u.)')
    ...
    _beautify_axes(ax2, title='RS – EDR (mean ± SEM, first 10 min)', 
                  xlabel='Time (mm:ss)')
    
    # Add overall figure title
    fig.suptitle('Electrodermal Response (EDR) by Task and Dose\nGroup Mean ± SEM across 11 Subjects', 
...
def plot_subject_edr_combined(subject: str) -> bool:
    ...
    dmt_high = load_cvx_edr(dmt_high_path)
    dmt_low = load_cvx_edr(dmt_low_path)
    if dmt_high is None or dmt_low is None:
        print(f"⚠️  Skipping {subject}: missing DMT data")
        return False

    t_high, edr_high = dmt_high
    t_low, edr_low = dmt_low
    ten_min = 600.0
    ...
    y_high_10 = edr_high[idx_h]
    y_low_10 = edr_low[idx_l]
    ...
    rs1 = load_cvx_edr(rs1_path)
    rs2 = load_cvx_edr(rs2_path)
    ...
    # Subplot 1: DMT high vs low - plot EDR directly without mean lines
    ...
    _beautify_axes(ax1, title='DMT High vs Low (first 10 min)', xlabel='Time', ylabel='EDR (a.u.)')
    ...
    # Subplot 2: RS ses01 vs ses02 with dose tags - plot EDR directly without mean lines
    ...
    fig.suptitle(f"{subject} – EDA EDR (first 10 min)", y=1.02)
    fig.tight_layout(pad=1.2)

    out_dir = os.path.join('test', 'eda', 'scr')
    os.makedirs(out_dir, exist_ok=True)
    out_path = os.path.join(out_dir, f"{subject.lower()}_eda_edr_amplitude_combined_10min.png")
    fig.savefig(out_path, dpi=200, bbox_inches='tight')
    plt.close(fig)
    print(f"✅ Saved combined EDR amplitude plot: {out_path}")
    return True


def main():
    print("📊 Generating EDA EDR combined plots per validated subject…")
    successes_combined = 0
    # Collect per-subject mean values for 2x2 ANOVA
    anova_records = []
    for subject in SUJETOS_VALIDADOS_EDA:
        ok = plot_subject_edr_combined(subject)
        if ok:
    ...
    report_path = os.path.join('test', 'eda', 'scr', 'anova_2x2_report.txt')
    if anova_records:
    ...
    if anova_records:
        out_dir = os.path.join('test', 'eda', 'scr')
        os.makedirs(out_dir, exist_ok=True)
        out_box = os.path.join(out_dir, 'edr_mean_2x2_boxplot.png')
        _plot_boxplot_2x2(anova_records, out_box)
        print(f"🖼️  2x2 mean EDR boxplot saved: {out_box}")

        # Combined summary plot with DMT and RS subplots
        out_sum_combined = os.path.join(out_dir, 'summary_combined_dmt_rs_mean_sem.png')
        ok_combined = _plot_combined_summary(out_sum_combined)
        if ok_combined:
