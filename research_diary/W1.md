...
[ ] Me esta faltando estandarizar la señal? Chequear. Deberia estar usando en algun momento algo tipo `nk.standardize(eda_signal)` pero tal vez `eda_process` ya lo hace directamente. A su vez, considerar que existe `baseline_correction = True` como metodo para el callculo de SCRs en neirokit2. Revisar.

[ ] Agregar en los scripts de `plot_eda_components.py` Salgun analisis de extraccion de CR counts (y/o amplitud de los SCRs) extraido de los archivos `emotiphai_scr.csv` guardados en `../data/derivatives/preprocessing/phys/eda/*`

[ ] Hacer espectrograma pero con los datos de EDA a nivel exploratorio, e integrarlo tambien en el documento .doc.

[ ] Modificar los archivos de EDR (columna `EDR` de `*cvx_decomposition.csv`) y EDL (columna `EDL` de `*cvx_decomposition.csv`) para que en vez de ceros (`0`) impute de una mejor forma (reemplazando por el ultimo valor valido de registro o dejandolo como `Nan` si lo acepta como tal al momento de hacer los analisis estadisticos). AL hacerlo, no reemplazar los archivos originales, porque eso me puede complicar cosas a futuro. Por el contrario, copiar y generar archivos nuevos con este reemplazo, para no arruinar analisis previos. Ir con cuidado.

[ ] Hacer analisis estadisticos de DMT (high vs low) con los 20 minutos (y no solo con los 10 minutos). Hacer esto para SMNA AUC, SCL, y SCR.

[ ] Rehacer las figuras y analisis estadisticos, con estos cambios.

[ ] Integrar esto cambios en `Borrador - Resultados DMT` todos los avances realizadso. Si hay analisis que no van en la historia principal, pero igual son relevantes, pueden ir en una seccion de "Anexo" del mismo documento.
