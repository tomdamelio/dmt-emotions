...
[ ] Revisar este error que tuve al correr @test_phys_preprocessing.py:    `⚠️  NeuroKit2 plotting failed: Length of values (112) does not match length of index (111)`. Esto me paso en al procesar al sujeto `s13` ara el archivo de la señal de EDA en la condicion dmt high. Por que sucede este error? Intentar plotearlo con neurokit2 para entender que sucede especificamente con este sujeto.

[ ] Chequear en @validation_log.json cuales son los archivos que fueron marcados como "bad", y cuales fueron marcados como "maybe" en "category". Los que fueron marcados como "maybe" quieor que me digas ademas cual es el valor de "note", para entender el motivo por el que se duda si incluirlos o no.

[ ] Con los datos nuevos de @validation_log.json quiero que me armes una lista para cada señal (eda, ecg y resp) con los sujetos validados para dicha señal. Para que un sujeto sea validado tiene que tener archivos validos (good o acceptable) en ambas condiciones de DMT (high dose y low dose).
Quiero que agregues esto como 3 nuevas listas en el config.py file (i.e. `SUJETOS_VALIDADOS_EDA`, `SUJETOS_VALIDADOS_ECG`, `SUJETOS_VALIDADOS_RESP`)

[ ] Buscar ahora cual es la mejor representacion para diferenciar entre dosis alta y dosis baja de EDA. Para empezar, hacer ploteos por participante comparando el AUC de SMNA (columna `SMNA` de `*cvx_decomposition.csv`) para el caso de dosis alta y dosis baja. Esto nos va a ayudar a explorar mejor los datos sujeto a sujeto (nos falta todavia ese insight).

[ ] Hacer tambien el plot para comparar dosis baja y dosis alta para el EDR (columna `EDR` de `*cvx_decomposition.csv`) y EDL (columna `EDL` de `*cvx_decomposition.csv`) por participante. 
